package com.seifmortada.applications.quran.ui.azkar_feature
...
private fun AzkarCard(
    ...
    Card(
        ...
        Column(
            ...
            val filteredZikr=zikr.text.replace("(","").replace(")","").replace("]","").replace("[","")
            Text(
                text = filteredZikr,
                fontSize = 20.sp,
...
@Composable
fun ButtonIcon(modifier: Modifier = Modifier, onClick: () -> Unit, iconId: Int, tintResource: Int) {
...
private fun AzkarScreenPreview() {
    ...
    AzkarScreen(azkars = fakeAzkarModel.array)
}