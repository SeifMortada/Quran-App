...
class ReciterSurahRecitationViewModel(
    ...
    fun fetchRecitation(
        ...
        try {
            ...
            // Build the download URL directly (same logic as RemoteDataSourceImpl)
            val formattedSurahNumber = String.format(Locale.US, "%03d", surahNumber)
            val normalizedServer = if (server.endsWith("/")) server else "$server/"
            val downloadUrl = "${normalizedServer}${formattedSurahNumber}.mp3"

            // Create the complete download request
            currentDownloadRequest = downloadRequest.copy(downloadUrl = downloadUrl)

            // Start the download using the app-wide download system
